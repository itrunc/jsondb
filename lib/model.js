...
const { keyValidate, objectDiff, retrieveObject, readJsonSync, outputJsonSync } = require('./helper')
...
class Model extends EventEmitter {
    ...
    #genFilePath() {
        ...
        const folder = path.resolve(this.#folder, `${year}`, `${month}${day}`, uid(2))
            ...
            try {
                ...
                validator.validate(data).then(() => {
                    ...
                    if (!isnew && this.#audit) {
                        ...
                        if (diff.k.length > 0) __audit__.push({ u: who, d: data.updatedAt, ...diff })
                        outputJsonSync(file, { ...data, __audit__ }, this.#encrypt)
                    } else {
    ...
    find(comparator = (obj) => { return false }, {
        ...
        let result = null
        const keys = Object.keys(this.#meta.data)
        let key = keys.pop()
        while(key) {
            const index = this.#meta.data[key]
            if (comparator(index)) {
                const file = this.getFilePath(key)
                const data = readJsonSync(file, this.#encrypt)
                if (data) {
                    result = { id: key, data: retrieveObject(data, { includeAllFields }), index }
                    key = undefined // break
                } else {
                    key = keys.pop()
                }
        ...
        while(key) {
            ...
            if (comparator(index)) {
                ...
                if (match > offset) {
                    const data = readJsonSync(file, this.#encrypt)
                    if (data) {
                        result.push({ id: key, data: retrieveObject(data, { includeAllFields }), index })
                        selected += 1
            ...
            try {
                ...
                if (data && Array.isArray(data) && data.length > 0) {
                    for (const { id = uid(8), ...value } of data) {
                        await this.set(id, value, { event, saveMeta: false, who, indexes: indexes[id] || {} }).then(data => {
     ...
     */
    buildIndex() {
        const debug = logger.extend('buildIndex')
        const objects = this.findAll()
        debug(`Retrieved ${objects.length} objects`)
        for (const { id, data, index } of objects) {
            this.#meta.data[id] = this.#genIndex(data)
        }
        debug('Build index done')
        this.saveMeta()
        debug('Saved meta')
    }
}
