...
const logger = require('debug')(`${packageName}:schema`)

const DEFAULT_PATH = path.resolve(os.homedir(), `.${packageName}-schema`)
    ...
    model(name, {
        ...
        try {
            ...
            if (this.#nameValidator(name)) {
                const folder = path.resolve(this.#modelfolder, name)
                const instance = new Model(folder, { rules, indexes })
    ...
    schema(name) {
        ...
        try {
            ...
            if (this.#nameValidator(name)) {
                ...
                console.log('schema', name, instance.isNew)
                if (instance.isNew) {
