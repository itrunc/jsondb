...
class JsondbUtils {
  ...
  async retrieveObjects(name, {
    ...
    const logs = { func, params: { name, criteria, pagination, year, version, user, safe }}
    try {
      const model = this.getModel(name, { year, user })
      ...
      this.logger.error(message, { ...logs, error })
  ...
  async retrieveMetaData(name, {
    ...
    const logs = { func, params: { name, criteria, year, user, safe }}
    try {
      const model = this.getModel(name, { year, user })
      if (!model) throw new Error(`Model ${name} is not found`)
  ...
  async saveObjects(name, data, { year, user = {}, indexes } = {}) {
    ...
    const logs = { func, params: { name, year, user, indexes }}
    try {
  ...
  async createObject(name, {
    ...
    const logs = { func, params: { name, year, user, override }}
    try {
      const model = this.getModel(name, { year, user })
      if (!model) throw new Error(`Model ${name} is not found`)
  ...
  async getObjects(name, idList, {
    ...
    const logs = { func, params: { name, year, user, idList, includeAllFields }}
    try {
  ...
  async updateObject(name, objectId, {
    ...
    const logs = { func, params: { name, year, user, objectId, create }}
    try {
      const model = this.getModel(name, { year, user })
      if (!model) throw new Error(`Model ${name} is not found`)
  ...
  async deleteObject(name, objectId, { year, user } = {}) {
    ...
    const logs = { func, params: { name, year, user, objectId }}
    try {
      const model = this.getModel(name, { year, user })
      if (!model) throw new Error(`Model ${name} is not found`)
  ...
  async deleteObjects(name, idList, { year, user, criteria } = {}) {
    ...
    const logs = { func, params: { name, year, user, idList, criteria }}
    try {
      let objects = []
