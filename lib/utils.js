...
class JsondbUtils {
  ...
  getModel(name, { user = {}, year } = {}) {
    year = year || this.year
    const logs = { func: 'getModel', params: { name, year, user }}
    this.logger.silly(`Get model ${name}`, { ...logs })
  ...
  removeModel(name, { user = {}, year } = {}) { // Dangerous: all objects in the model will been removed
    year = year || this.year
    const logs = { func: 'removeModel', params: { name, year, user }}
    this.logger.silly(`Remove model ${name}`, { ...logs })
    const userid = user.id
    const def = this._models[name] || this.name === 'temp' && { name }
    if (!def) {
      this.logger.warn(`Model ${this.appid}.${this.name}.${name} is not defined`, { ...logs })
  ...
  checkObjectExistence(name, idOrComparison, { year, user } = {}) {
    ...
    const logs = { func: 'checkObjectExistence', params: { name, objectId, year, user }}
    this.logger.silly(`Check object existence in model ${name}`, { ...logs })
  ...
  async retrieveObjects(name, {
    ...
    this.logger.silly(`Get objects from model ${name}`, { ...logs })
    try {
      const model = this.getModel(name, { year, user })
      if (!model) throw new Error(`Model ${name} is not found`)
      return new Promise((resolve, reject) => {
        model.findAllPromise(item => comparison(item, criteria, Object.keys(item)), {
          ...pagination, includeAllFields
        }).then(objects => {
          let mapped = objects.map(({ id, data = {}, index } = {}) => {
            return { id, ...data }
          })
          this.logger.info(`${mapped.length} objects are retrieved from ${name}`, { ...logs })
          if (safe) {
            mapped = mapped.filter(item => comparison(item, criteria, Object.keys(criteria)))
            this.logger.info(`${mapped.length} objects are retrieved from ${name} with safe mode`, { ...logs })
          }
          if (version) {
            resolve({ version: model.version, objects: mapped })
          } else {
            resolve(mapped)
          }
        }).catch(reject)
  ...
  async retrieveMetaData(name, {
    ...
    this.logger.silly(`Get metadata from model ${name}`, { ...logs })
    try {
      const model = this.getModel(name, { year, user })
      if (!model) throw new Error(`Model ${name} is not found`)
  ...
  async saveObjects(name, data, { year, user = {}, indexes } = {}) {
    ...
    this.logger.silly(`Import objects to model ${name}`, { ...logs })
    try {
  ...
  async createObject(name, {
    ...
    this.logger.silly(`Create object in model ${name}`, { ...logs })
    try {
      const model = this.getModel(name, { year, user })
      if (!model) throw new Error(`Model ${name} is not found`)
  ...
  async getObject(name, objectId, {
    ...
    this.logger.silly(`Get object ${objectId} from model ${name}`, { ...logs })
    try {
      const model = this.getModel(name, { year, user })
      if (!model) throw new Error(`Model ${name} is not found`)
  ...
  async getObjectAudits(name, objectId, { year, user } = {}) {
    ...
    this.logger.silly(`Get audits of object ${objectId} from model ${name}`, { ...logs })
    try {
      const model = this.getModel(name, { year, user })
      if (!model) throw new Error(`Model ${name} is not found`)
      const item = objectId && typeof objectId === 'string' && model.getAudit(objectId) || {}
  ...
  async getObjects(name, idList, {
    ...
    this.logger.silly(`Get objects from model ${name} by id list`, { ...logs })
    try {
      if (!Array.isArray(idList)) throw new TypeError(`Invalid ID list ${typeof idList}`)
      return Promise.allSettled(idList.map(objectId => this.getObject(name, objectId, { year, user, includeAllFields }), this)).then(results => {
        return results.reduce((acc, { status, value, reason } = {}, index) => {
          if (/fulfilled/i.test(status) && value) {
            acc.success.push(value)
          } else {
            acc.failure.push({ id: idList[index], reason: reason || 'Not Found', status, value })
          }
          return acc
        }, {
          success: [],
  ...
  async updateObject(name, objectId, {
    ...
    this.logger.silly(`Update object ${objectId} in model ${name}`, { ...logs })
    try {
      const model = this.getModel(name, { year, user })
      if (!model) throw new Error(`Model ${name} is not found`)
  ...
  async deleteObject(name, objectId, { year, user } = {}) {
    ...
    this.logger.silly(`Delete object ${objectId} from model ${name}`, { ...logs })
    try {
      const model = this.getModel(name, { year, user })
      if (!model) throw new Error(`Model ${name} is not found`)
  ...
  async deleteObjects(name, idList, { year, user, criteria } = {}) {
    ...
    this.logger.silly(`Delete objects from model ${name}`, { ...logs })
    try {
      let objects = []
      ...
      if (objects.length) {
        return Promise.allSettled(objects.map(obj => this.deleteObject(name, obj.id, { year, user }), this)).then(results => {
          return results.reduce((acc, { status, value, reason } = {}, index) => {
            if (/fulfilled/i.test(status) && value) {
              acc.success.push(value)
            } else {
              const obj = objects[index] || {}
              acc.failure.push({ id: obj.id, reason: reason || 'Not Found', status, value })
            }
            return acc
          }, deleted)
        })
