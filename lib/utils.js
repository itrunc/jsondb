...
class JsondbUtils {
  constructor(instance, appid, name, {
    ...
    if (logger) {
      ...
      this.logger = {
        silly: createLogger('utils:silly'),
        debug: createLogger('utils:debug'),
        info: createLogger('utils:info'),
        warn: createLogger('utils:warn'),
        error: createLogger('utils:error')
      }
    }
  ...
  async getObject(name, objectId, {
    ...
    try {
      ...
      if (item) {
        ...
        this.logger.info(`Object ${name}(${objectId}) is fetched`, { ...logs })
      } else {
